...
describe('Markdown report', () => {
  it('renders an empty report with disclaimer and statistics', () => {
    const report = renderMarkdown({ pullRequest: pullRequestFixture, result: resultFixture({ findings: [], riskLevel: 'low' }), skippedFiles: [], reviewedFileCount: 2 });
  ...
  it('renders finding details and skipped files', () => {
    const report = renderMarkdown({ pullRequest: pullRequestFixture, result: resultFixture({ findings: [findingFixture(), findingFixture({ severity: 'medium', title: 'Missing test' })] }), skippedFiles: [{ path: 'logo.png', reason: 'binary' }], reviewedFileCount: 1 });
